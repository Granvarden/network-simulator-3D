...
class OverviewHUD:
    ...
    def render(
        ...
        card_y = self.screen_h - self.card_h - 48

        # Card Background (Translucent dark slate with rounded corners)
        ui.draw_rect(card_x, card_y, self.card_w, self.card_h, (15, 23, 42), alpha=0.94, corner_radius=14.0)
        ...
        if target.device:
            ...
            ui.draw_text(
                f"● {power_str}",
                card_x + self.card_w - 20, card_y + 24,
                font_size=14,
                color=power_col,
                center_x=False
            )
            ...
            # Divider line
            ui.draw_rect(card_x + 18, card_y + 68, self.card_w - 36, 1, (51, 65, 85), alpha=0.8)

            # Active targeted port highlight banner (if aiming directly at a port)
            if target.port:
                p = target.port
                p_box_y = card_y + 76
                ui.draw_rect(card_x + 16, p_box_y, self.card_w - 32, 54, (30, 41, 59), alpha=0.92, corner_radius=8.0)
                ui.draw_rect_outline(card_x + 16, p_box_y, self.card_w - 32, 54, (56, 189, 248), line_width=1.2)

                # Port name and Admin/Link status
                status_color = (74, 222, 128) if p.is_operational else (239, 68, 68)
                ui.draw_text(
                    f"Target Port: {p.port_name}",
                    card_x + 24, p_box_y + 16,
                    font_size=15,
                    color=(255, 255, 255)
                )
                ui.draw_text(
                    f"[{p.admin_status.value.upper()} / {p.link_status.value.upper()}]",
                    card_x + 200, p_box_y + 16,
                    font_size=14,
                    color=status_color
                )
                ...
                ui.draw_text(conn_text, card_x + 24, p_box_y + 36, font_size=13, color=conn_col)

                ports_start_y = card_y + 138
                max_list = 4
            else:
                ports_start_y = card_y + 78
                max_list = 6

            # List of ports overview
            ui.draw_text("Ports Overview:", card_x + 18, ports_start_y, font_size=14, color=(148, 163, 184))
            port_y = ports_start_y + 20
            ...
            for p in ports:
                ...
                ui.draw_text(f"●", card_x + 20, port_y, font_size=12, color=dot_col)
                ...
                ui.draw_text(p_str, card_x + 36, port_y, font_size=13, color=(226, 232, 240))
                port_y += 20
            ...
            # Footer / Action Hints
            ui.draw_rect(card_x + 18, card_y + self.card_h - 40, self.card_w - 36, 1, (51, 65, 85), alpha=0.8)
            if mode == "CABLING_CLI":
                if held_cable_port:
            ...
            ui.draw_text(
                hint,
                card_x + self.card_w / 2.0, card_y + self.card_h - 20,
                font_size=14,
                color=hint_col,
                center_x=True,
                center_y=True
        ...
        # 2. When aiming at an empty Rack slot
            ...
            ui.draw_rect(card_x + 18, card_y + 74, self.card_w - 36, 1, (51, 65, 85), alpha=0.8)
            ...
            ui.draw_text(
                ...
                card_x + 18, card_y + 118,
                font_size=13,
                color=(148, 163, 184)
            )
            ui.draw_text(
                "Supports Router (2U), Switch (1U), or PC (2U).",
                card_x + 18, card_y + 140,
                font_size=13,
                color=(148, 163, 184)
            )
